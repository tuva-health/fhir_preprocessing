...
select distinct
      cast({{ fhir_preprocessing.fhir_patient_internal_id('condition') }} as {{ dbt.type_string() }} ) as patient_internal_id
    ...
    , case
        when lower(condition.normalized_code_type) = 'icd-10-cm'
            ...
            then cast({{ the_tuva_project.concat_custom([
...
from {{ ref('fhir_preprocessing__stg_core__condition') }} as condition
    left outer join {{ ref('fhir_preprocessing__stg_core__encounter') }} as encounter
        ...
        and condition.data_source = encounter.data_source
